...
func (o *GetClassURL) Build() (*url.URL, error) {
	...
	var _path = "/class/{playerId}"

	playerID := swag.FormatInt64(o.PlayerID)
	if playerID != "" {
		_path = strings.Replace(_path, "{playerId}", playerID, -1)
	} else {
		return nil, errors.New("playerId is required on GetClassURL")
	}
...
func (o *GetClassURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetClassURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetClassURL")
	}
